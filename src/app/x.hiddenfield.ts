...
@Component({
  selector: 'x-' + xhiddenfieldMetaData.XTYPE,
	...
	providers: [{provide: xbase, useExisting: forwardRef(() => xhiddenfield)}],
	template: '<template #dynamic></template>'
})
export class xhiddenfield extends xbase {
	constructor(eRef:ElementRef,resolver:ComponentFactoryResolver,vcRef:ViewContainerRef) {
		super(eRef,resolver,vcRef,xhiddenfieldMetaData);
	}
	@ContentChildren(xbase,{read:ViewContainerRef}) extbaseRef:QueryList<ViewContainerRef>;
	@ViewChild('dynamic',{read:ViewContainerRef}) dynamicRef:ViewContainerRef;
	ngAfterContentInit() {this.AfterContentInit(this.extbaseRef);}
	ngOnInit() {this.OnInit(this.dynamicRef,xhiddenfieldMetaData);}
}
